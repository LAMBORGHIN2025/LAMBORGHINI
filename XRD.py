...
class MixNet(object):
    ...
    def Message_Traveling(self,message):
        ...
        #print((To_list(self.Latency_List[0])[Client_ID][Ch1]))
        self.env.timeout(To_list(self.Latency_List[0])[Client_ID][Ch1])
        
        #message.chain = Chain1
        #message.next = True
        #Chain1[0].Mixes = self.M
        for ii in range(self.hops):
            yield self.env.process(Chain1[ii].Receive(message))#Mixing delay
            
        self.LL0.append(To_list(self.Latency_List[0])[Client_ID][Ch1]+0.3)#The latency will be added to the latency list
        #self.LL0.append(self.env.now-message.mtime[0])    
        ...
        for itt in range(self.hops):
            yield self.env.process(Chain2[itt].Receive(message))#Mixing delay
        message.chain = Chain2
        #message.next = True
        Chain2[0].Mixes = self.M

        yield self.env.process(Chain2[0].Receive(message))#Mixing delay
        ...
        self.LL.append((To_list(self.Latency_List[0])[Client_ID][Ch1]+0.6+To_list(self.Latency_List[1])[Ch1][Ch2]))#The latency will be added to the latency list
        #self.LL.append(message.mtime[1]-message.mtime[0])
