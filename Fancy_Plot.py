       ...
       # self.colors[2] = color1
        ...
        for j in range(len(self.Y)):
            ...
            for i in range(len(self.X)):
                position = 2*i + 0.5 * j  # Adjust the multiplier as needed for proper spacing
        ...
        Elements2 = []         
        for jj in range(2):
            hatch_pattern = '' if jj == 0 else '/' if jj == 1 else '\\'  # Define hatch patterns
            # Add a legend entry for the first half
            legend_entry1 = LegendHandler(color=self.colors[jj], label=self.Desc[jj], hatch_pattern=hatch_pattern)
            Elements1.append(legend_entry1)
            
            # Add a legend entry for the second half
            legend_entry2 = LegendHandler(color=self.colors[jj+2], label=self.Desc[jj+2], hatch_pattern=hatch_pattern)
            Elements2.append(legend_entry2)
        
        # Set and display the first legend on the left
        legend1 = axs.legend(handles=Elements1, fontsize=15, loc='lower left')
        # Add the first legend to the axes
        axs.add_artist(legend1)
        
        # Set and display the second legend on the right
        legend2 = axs.legend(handles=Elements2, fontsize=15, loc='lower right')

       # axs.legend(handles=legend_elements, fontsize=12, loc='upper left')
        #plt.legend(handles=Elements, fontsize=12, loc='upper left')
        #plt.legend(handles=Elements2, fontsize=15, loc='lower right')           
        if Grid:
            plt.grid(linestyle='--', color='darkblue', linewidth=1.5)
        ...
        x_positions = [i*2+0.75 for i in range(len(self.X)) ]
